...
namespace HoloToolkit.Unity.SpatialMapping
{
    ...
    public class UR5TapToPlace : MonoBehaviour, IInputClickHandler
    {
        ...
        protected virtual void Update()
        {
            ...
            {
                Debug.Log("Wir wollen den Roboter drehen!");
                Vector3 newPlacementPosition = new Vector3(placementPosition.x, this.transform.position.y, placementPosition.z);
                Vector3 rotateVector = newPlacementPosition - this.transform.position;
                interpolator.SetTargetRotation(Quaternion.LookRotation(rotateVector, Vector3.up));
        ...
        {
            Debug.Log("Wir bewegen den UR5");

            // On each tap gesture, toggle whether the user is in placing mode.
            if (IsBeingPlaced == true)
            {
                if(isBeingRotated == false)
                {
                    IsBeingPlaced = false;
            ...
            } else
            {
                ...
                {
                    isBeingRotated = !isBeingRotated;
                }else if (isBeingRotated == false)
                {
                    IsBeingPlaced = !IsBeingPlaced;
                }
            }
            /*
            else if(!isBeingRotated)
            {
                isBeingRotated = !isBeingRotated;
            }
        ...
        private void HandlePlacement()
        {
            ...
            else if (IsBeingPlaced == false && isBeingRotated == true)
            {
                Debug.Log("--StartStopRotating--");
                //StopPlacing();
                //StartStopRotating();
            }
            else if (IsBeingPlaced == false && isBeingRotated == false)
            {
                StopPlacing();
        ...
        private void StartStopRotating()
        {
            var layerCacheTarget = PlaceParentOnTap ? ParentGameObjectToPlace : gameObject;
            layerCacheTarget.SetLayerRecursively(IgnoreRaycastLayer, out layerCache);
            InputManager.Instance.PushModalInputHandler(gameObject);
