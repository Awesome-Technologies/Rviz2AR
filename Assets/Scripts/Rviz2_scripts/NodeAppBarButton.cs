...
namespace HoloToolkit.Unity.UX
{
    ...
    /// </summary>
    public class NodeAppBarButton : MonoBehaviour
    {
        private ButtonIconProfile customIconProfile;
        private NodeAppBar.ButtonTemplate template;
        ...
        private NodeAppBar parentToolBar;
        ...
        protected void Update()
        {
            ...
            {
                case NodeAppBar.AppBarStateEnum.Default:
                    // Show hide, adjust, remove buttons
                    // The rest are hidden
                    targetPosition = defaultOffset;
                    switch (template.Type)
                    {
                        case NodeAppBar.ButtonTypeEnum.Hide:
                        case NodeAppBar.ButtonTypeEnum.Remove:
                        case NodeAppBar.ButtonTypeEnum.Adjust:
                        case NodeAppBar.ButtonTypeEnum.Custom:
                            Show();
                            break;
                ...
                case NodeAppBar.AppBarStateEnum.Hidden:
                    // Show show button
                    // The rest are hidden
                    targetPosition = hiddenOffset;
                    switch (template.Type)
                    {
                        case NodeAppBar.ButtonTypeEnum.Show:
                            Show();
                            break;
                ...
                case NodeAppBar.AppBarStateEnum.Manipulation:
                    // Show done / remove buttons
                    // The rest are hidden
                    targetPosition = manipulationOffset;
                    switch (template.Type)
                    {
                        case NodeAppBar.ButtonTypeEnum.Done:
                        case NodeAppBar.ButtonTypeEnum.Remove:
                            Show();
                            break;
